...
export const Step2Director: React.FC<Step2Props> = ({ config, onUpdate, onBuyCredits, onTestMode, onBack }) => {
  ...
  const savePreset = () => {
    ...
    const newPreset: DirectorPreset = {
        ...
        createdAt: Date.now(),
        config: {
            selectedStyleId: config.selectedStyleId,
            intensity: config.intensity,
            duration: config.duration,
            smoothness: config.smoothness,
            stutter: config.stutter,
            stutterPreset: config.stutterPreset,
            ...
            morphIntensity: config.morphIntensity,
            superModeMotionPreset: config.superModeMotionPreset,
        }
    };
    
    const updatedPresets = [...(config.savedDirectorPresets || []), newPreset];
    onUpdate('savedDirectorPresets', updatedPresets);
    setNewPresetName('');
    setShowPresets(false);
  };

  const loadPreset = (preset: DirectorPreset) => {
      triggerImpulse('click', 1.0);
  ...
  const deletePreset = (id: string, e: React.MouseEvent) => {
      e.stopPropagation();
      triggerImpulse('click', 0.5);
      const updatedPresets = config.savedDirectorPresets.filter(p => p.id !== id);
      onUpdate('savedDirectorPresets', updatedPresets);
  };
  ...
  const STUTTER_PRESETS: { id: StutterMode, label: string }[] = [
      { id: 'auto', label: '🎲 Auto / Random' },
      { id: 'shiver', label: '🫨 Shiver (Horizontal)' },
      { id: 'jump', label: '⬆️ Jump (Vertical)' },
      { id: 'smash', label: '👊 Smash (Zoom Pulse)' },
      { id: 'slice', label: '🔪 Slice (Glitch/Tear)' }
  ];
             ...
             <div className="flex gap-2 items-center justify-between md:justify-end overflow-x-auto pb-1">
                ...
                {showPresets && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-black/95 backdrop-blur-xl border border-white/20 rounded-xl p-4 shadow-2xl z-50 animate-fade-in">
                        <div className="mb-4">
                            <label className="text-[10px] text-gray-500 font-bold uppercase block mb-2">Save Current Config</label>
                            <div className="flex gap-2">
                                <input 
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:border-brand-500 outline-none"
                                />
                                <button onClick={savePreset} disabled={!newPresetName} className="bg-brand-600 hover:bg-brand-500 text-white p-1.5 rounded-lg disabled:opacity-50">
                                    <Save size={14}/>
                                </button>
                            </div>
                        </div>
                        
                        {config.savedDirectorPresets?.length > 0 && (
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                <label className="text-[10px] text-gray-500 font-bold uppercase block mb-1">Load Preset</label>
                                {config.savedDirectorPresets.map(preset => (
                                    <div key={preset.id} onClick={() => loadPreset(preset)} className="flex justify-between items-center p-2 rounded-lg bg-white/5 hover:bg-white/10 cursor-pointer group">
                                        <span className="text-xs font-bold text-gray-300 group-hover:text-white truncate">{preset.name}</span>
                                        <button onClick={(e) => deletePreset(preset.id, e)} className="text-gray-600 hover:text-red-400 p-1">
                                            <Trash2 size={12}/>
                                        </button>
                                    </div>
                                ))}
                        ...
                        {(!config.savedDirectorPresets || config.savedDirectorPresets.length === 0) && (
                            <p className="text-[10px] text-gray-600 italic text-center py-2">No saved presets yet.</p>
                        )}
                    </div>
                )}
            </div>
          ...
          <div className={`transition-all duration-500 ease-in-out overflow-hidden ${showAdvanced ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-white/5 bg-black/20">
                  ...
                  <div className="space-y-6">
                      ...
                      {/* SUPER MODE PRESETS DROPDOWN */}
                      {config.superMode && (
                        <div className="animate-fade-in p-3 rounded-xl border border-yellow-500/20 bg-yellow-900/10">
                            <label className="text-xs font-bold text-yellow-500 mb-2 block uppercase tracking-wider flex items-center gap-2">
                                <Star size={10} fill="currentColor"/> Super Mode Choreography
                            </label>
                            <div className="relative">
                                <select 
                                    value={config.superModeMotionPreset || 'triplet-flow'}
                                    onChange={(e) => onUpdate('superModeMotionPreset', e.target.value)}
                                    className="w-full bg-black/40 border border-yellow-500/30 text-white text-xs font-bold rounded-lg p-3 appearance-none focus:border-yellow-500 outline-none"
                                >
                                    {SUPER_MODE_PRESETS.map(p => (
                                        <option key={p.id} value={p.id}>{p.label}</option>
                                    ))}
                                </select>
                                <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-yellow-500/50 pointer-events-none"/>
                            </div>
                            <p className="text-[10px] text-gray-400 mt-2 italic">
                                {SUPER_MODE_PRESETS.find(p => p.id === config.superModeMotionPreset)?.description}
                            </p>
                        </div>
                      )}
                  ...
                  <div className="space-y-6 border-l border-white/5 pl-8 md:pl-8">
                       ...
                       <div className="space-y-4 pt-4 border-t border-white/5">
                            {/* Motion Smoothness */}
                            <div>
                                <label className="text-xs font-bold text-gray-400 mb-2 block uppercase tracking-wider">Motion Smoothness</label>
                                <div className="flex items-center gap-3">
                                    <span className="text-[10px] text-gray-600">CUTS</span>
                                    <input 
                                        type="range" min="0" max="100" 
                                        value={config.smoothness} 
                                        onChange={(e) => onUpdate('smoothness', Number(e.target.value))}
                                        className="w-full h-1.5 bg-white/10 rounded-full accent-blue-400 cursor-pointer"
                                    />
                                    <span className="text-[10px] text-gray-600">BLEND</span>
                                </div>
                                <div className="text-right text-xs text-blue-300 font-mono mt-1">
                                    {config.smoothness < 30 ? 'Hard Cuts' : config.smoothness > 70 ? 'Liquid Flow' : 'Standard'}
                                </div>
                            </div>

                            {/* Stutter Interval & Style (Refactored) */}
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                                        <BarChart2 size={12}/> Stutter Rhythm
                                    </label>
                                    {/* Stutter Style Dropdown */}
                                    <div className="relative">
                                        <select 
                                            value={config.stutterPreset || 'auto'} 
                                            onChange={(e) => onUpdate('stutterPreset', e.target.value)}
                                            className="bg-black/40 border border-white/10 text-[10px] text-white rounded px-2 py-1 appearance-none focus:border-red-400 outline-none pr-6 cursor-pointer"
                                        >
                                            {STUTTER_PRESETS.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
                                        </select>
                                ...
                                <input 
                                    type="range" min="0" max="100" 
                                    value={config.stutter} 
                                    onChange={(e) => onUpdate('stutter', Number(e.target.value))}
                                    className="w-full h-1.5 bg-white/10 rounded-full accent-red-400 cursor-pointer"
                                />
                                <div className="flex justify-between mt-1 text-[10px] font-mono">
                                    <span className="text-gray-600">RARE</span>
                                    <span className="text-red-300">{config.stutter}% Density</span>
                                    <span className="text-gray-600">FREQUENT</span>
                                </div>
                                <p className="text-[10px] text-gray-500 mt-1">Controls the interval between stutters. Higher = more frequent glitches.</p>
                            </div>
                       </div>
                  </div>
              </div>
